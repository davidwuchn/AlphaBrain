#!/bin/bash
# Pi05 5traj variant of run_rlt_ori_rl_task0_release_pi05_1traj.sh.
# Same hyperparams + steplock, only swaps VLA backbone and Phase-1 encoder
# to the Pi05 5traj pair (matches the 5traj VLA + 5traj rlt_ori encoder).
#
# Usage:
#   bash scripts/run_rl_scripts/run_rlt_ori_rl_task0_release_pi05_5traj.sh [GPU_ID]
set -euo pipefail
...
ENCODER_PATH="results/rlt_ori_training/pi05_5traj_openpi_strict_0427_0721/pretrain/checkpoints/pretrain_best/encoder.pt"

RUN_NAME="rlt_ori_rl_t${TASK_ID}_release_pi05_5traj"
...
if [ ! -f "${ENCODER_PATH}" ]; then
    ...
    echo "       (Pi05 5traj Phase-1 may still be running.)"
    exit 1
fi

echo "============================================================"
echo " RLT_ori Phase-2 TD3 (release-pi05-5traj, libero_goal task ${TASK_ID})"
echo "   GPU:        ${GPU_ID}"
echo "   ckpt:       ${CKPT_PATH}"
